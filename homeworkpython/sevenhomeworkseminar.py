...
# '''
#
pooh = input('Введите слова песни на русском языке: ')
print(pooh)
pooh_l = list(pooh.split())
gl_lit = list('ёуеыаоэяию')
gl_lit_pooh = list()
for phrase in pooh_l:
    k = 0
    for lett in phrase:
        if lett in gl_lit:
            k += 1
    gl_lit_pooh.append(k)
if len(set(gl_lit_pooh)) == 1:
    print('Парам пам-пам')
else:
    print('Пам парам')
'''
